...
def trainer(config, training):
    with tempfile.TemporaryDirectory() as temp_model_dir:
        start_time = time.time()
        model_path = train_nlu(config=config, nlu_data=training, output=temp_model_dir, additional_arguments = {})
...
def train(config, training):
    duration = trainer(config, training)
